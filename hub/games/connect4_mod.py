...
class Connect4_Mod(Game):
    ...
    def flip(self,move):
        if self.gameboard[move] == 3 - self.rep[self.turn]:
            self.gameboard[move] = self.rep[self.turn]
            return True
        else:
            return False
            ...
            for j in range(self.gameboard.shape[1]):
                if not self.flipping:
                    if (self.boardrects[i][j]).collidepoint(click.pos) and np.argwhere(self.gameboard[:,j] == 0).size != 0:
                        self.make_move((i,j))
                        if self.check4((np.argwhere(self.gameboard[:,j] != 0)[0,0],j)):
                            self.points[self.rep[self.turn]] += 4
                            self.flipping = True
                        elif self.check_win_condition(): # checks win condition where move was played not where mouse was clicked
